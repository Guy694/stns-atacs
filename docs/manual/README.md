...
### 5.4 การนำเข้าครุภัณฑ์ด้วย CSV

**สิทธิ์ที่ต้องมี:** `assets.create` สำหรับการเพิ่มใหม่ และ `assets.update` สำหรับการแก้ไขข้อมูลเดิม

1. ไปที่ **รายการทรัพย์สิน** และคลิก **นำเข้า CSV**
2. ดาวน์โหลด **ไฟล์ CSV ตัวอย่าง** จากหน้าต่างนำเข้า แล้วกรอกข้อมูลโดยคงชื่อคอลัมน์แถวแรกไว้เหมือนเดิม
3. เลือกหน่วยบริการให้ตรงกับข้อมูลในไฟล์ ระบบจะแสดงตาราง **Work Group ของหน่วยบริการ** พร้อมชื่อกลุ่มงานและ `work_group_id` ที่ใช้งานได้
4. คัดลอก `work_group_id` ของกลุ่มงานที่ถูกต้องใส่ในไฟล์ CSV เมื่อหน่วยบริการนั้นมีกลุ่มงาน ห้ามใช้ id ของกลุ่มงานจากหน่วยบริการอื่น
5. เลือกไฟล์ `.csv` ที่บันทึกเป็น **UTF-8** ระบบรองรับ `.xlsx` และ `.xls` เพื่อให้ใช้กับไฟล์เดิมได้ด้วย
6. ตรวจสอบผลการนำเข้า ซึ่งจะแสดงจำนวนรายการที่เพิ่มใหม่ แก้ไข และข้าม พร้อมสาเหตุของแถวที่นำเข้าไม่สำเร็จ

> **หลักการของคอลัมน์ `id`:** เว้น `id` ว่างเมื่อต้องการเพิ่มครุภัณฑ์ใหม่ ระบบจะสร้าง id ให้เองหลังบันทึกข้อมูล แต่หากต้องการแก้ไขครุภัณฑ์ที่มีอยู่ ต้อง Export CSV จากระบบก่อน แล้วคงค่า `id` ของรายการนั้นไว้ในไฟล์นำเข้า

| กรณีใช้งาน | ค่าในคอลัมน์ `id` | ผลลัพธ์ |
|---|---|---|
| เพิ่มครุภัณฑ์ใหม่ | เว้นว่าง | ระบบสร้างรายการใหม่และกำหนด id อัตโนมัติ |
| แก้ไขครุภัณฑ์เดิม | ใส่ id ที่มาจากไฟล์ Export ของระบบ | ระบบแก้ไขรายการที่ตรงกับ id นั้น |
| ใส่ id ที่ไม่มีในระบบ | ใส่ตัวเลขที่ไม่พบ | ระบบข้ามแถวนั้นและแจ้งว่าไม่พบครุภัณฑ์ |
| ใส่ id ของหน่วยบริการอื่น | ใส่ id ที่ไม่อยู่ในหน่วยบริการที่เลือก | ระบบข้ามแถวนั้นเพื่อป้องกันการแก้ไขข้ามหน่วยบริการ |

คอลัมน์ที่ต้องมีคือ `asset_name` ส่วน `asset_class` ใช้ค่า `IT`, `Office`, `Medical`, `Vehicle`, `Building`, `Utility` หรือ `Other` และ `asset_category` ใช้ `Hardware` หรือ `Software` สำหรับครุภัณฑ์คอมพิวเตอร์ ให้ระบุ `windows_license_status` เป็น `Genuine` หรือ `Pirated` เสมอ โดยใช้วันที่ในรูปแบบ `YYYY-MM-DD` สำหรับ `work_group_id` ให้ดูตัวเลขจากตารางที่ระบบแสดงหลังเลือกหน่วยบริการ หากตารางแจ้งว่าไม่มีกลุ่มงานที่เปิดใช้งาน สามารถเว้นคอลัมน์นี้ว่างได้

### 5.5 ข้อความตรวจสอบที่พบบ่อย
...
| ไม่พบครุภัณฑ์ตาม id ที่ระบุ | Export CSV ใหม่จากระบบ และใช้ id ของรายการเดิมโดยไม่แก้ไข |
| id นี้ไม่ได้อยู่ในหน่วยบริการที่เลือก | เลือกหน่วยบริการให้ตรงกับข้อมูลเดิม หรือแยกไฟล์ตามหน่วยบริการ |
