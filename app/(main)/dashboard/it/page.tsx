...
function summarizeComputerLicenses(
  ...
  return assets.reduce<ComputerLicenseSummary>(
    (summary, asset) => {
      if (!isItAsset(asset) || asset.assetGroup !== "Hardware" || !isComputerDeviceType(asset.deviceType)) return summary;
...
export default async function ItDashboard({ searchParams }: HomeProps) {
  ...
  const computerViewHref = (view: "district" | "facility", district?: string) => {
    ...
    return `/dashboard/it?${query.toString()}`;
  };
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1600px] flex-1 flex-col gap-6">
        ...
        <form method="GET" className="glass-panel rounded-2xl p-3 sm:p-4" aria-label="ตัวกรองข้อมูลภาพรวม">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid min-w-0 gap-3 md:grid-cols-[1fr_1fr_1.35fr_auto] md:items-end">
              ...
              <button
                type="submit"
                className="min-h-9 rounded-[10px] bg-[var(--accent-strong)] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#125b86] md:mb-0"
              >
                กรองข้อมูล
              </button>
            </div>
