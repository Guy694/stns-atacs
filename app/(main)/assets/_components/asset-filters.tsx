...
const control = "filter-control w-full";
...
export function AssetFilters({ values, resetHref, subtypes, deviceTypes, facilities, districts, workGroups = [], paginate = false }: Props) {
  ...
  return <form method="GET" action={resetHref} className="min-w-0 space-y-4 rounded-xl border border-[var(--line)] bg-white p-3 sm:p-4" aria-label="ค้นหาและกรองครุภัณฑ์">
    <div className="flex flex-wrap items-center justify-between gap-2">
      <h2 className="text-base font-semibold">ค้นหาและกรองครุภัณฑ์</h2>
      {activeCount > 0 && <a href={resetHref} className="inline-flex min-h-9 items-center text-xs text-[var(--primary-text)] underline underline-offset-4">ล้างตัวกรอง ({activeCount})</a>}
    </div>
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,2fr)_minmax(0,1.5fr)_minmax(0,1fr)]">
    ...
    <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
      ...
      <button type="submit" className="filter-button bg-[var(--accent-strong)] text-white hover:bg-[var(--primary-hover)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)]">ค้นหา</button>
    </div>
    <details open={advancedCount > 0 || (paginate && Boolean(values.perPage))} className="border-t border-[var(--line)] pt-3">
      <summary className="w-fit cursor-pointer py-2 text-xs font-medium text-[var(--primary-text)] focus-visible:outline-2 focus-visible:outline-[var(--accent)]">ตัวกรองเพิ่มเติม{advancedCount > 0 ? ` · ใช้อยู่ ${advancedCount} เงื่อนไข` : ""}</summary>
